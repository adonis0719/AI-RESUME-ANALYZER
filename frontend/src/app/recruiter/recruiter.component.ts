...
export class RecruiterComponent implements OnInit {
  ...
  scheduleInterview() {
    const selected = this.results.filter((r: any) => r.selected);
    const emails = selected
      .map((r: any) => r.email)
      .filter((email: any) => email && String(email).trim());
    ...
    if (!emails.length) {
      alert('Selected resumes do not have extracted email addresses. Please re-analyze the resumes to extract emails from the PDFs.');
      return;
    }
